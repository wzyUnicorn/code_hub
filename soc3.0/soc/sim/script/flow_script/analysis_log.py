...
def analysis_sim_log(file='',dir=''):
    ...
    for line in content:
        if(ERROR_LEVEL==1):
            ...
            if(re.search("(Error)|(error)|(FATAL)|(fail)|(UVM_ERROR)|(UVM_FATAL)",line,re.I)!=None):   
                if(re.search("UVM_ERROR :.*(\d+)|(UVM_FATAL :.*(\d+))",line)==None):
                    if(error_waiver_get(line)):
                        fail_reason = line.replace("\n",'')
                        fail = 1
    ...
    test_file = tmp.group(1)
    if(fail==1):
        report_sim_f.write(cmd_args.case+ ":FAIL        Fail message: "+ fail_reason+",seed=" + cmd_args.seed + "\n")
        report_tmp_sim_f.write(test_name + (30-len(test_name)) * " " + "COMPILE FAIL" + 18*" " +  cmd_args.seed + (30-len( cmd_args.seed))*' ' + fail_reason+"\n")
    else:
        report_sim_f.write(cmd_args.case+ ":PASS        \n")
        report_tmp_sim_f.write(test_name + (30-len(test_name)) * " " + "PASS" + 22*" " +  cmd_args.seed + (30-len( cmd_args.seed))*' '+"\n")
    report_sim_f.close()
...
def waiver_get():
    ...
    #waiver_message= {}
    waiver_message["all"] = []
    waiver_f = open(PRJ_ROOT+"/script/flow_script/error_waiver.txt","r")
...
def error_waiver_get(line ='',test_file=''):
    ...
    if re.search("(UVM_(FATAL|ERROR)\s+reports\s+:\s+0)",line):
        result = 0

    if re.search("(UVM_(FATAL|ERROR)\s+:\s+0)",line):
        result = 0
    ...
    for test_file in waiver_message:
        if(test_file=="all"):
            ...
            if test_file_sel in waiver_message:
                    ...
                    if(test == "all"):
                        ...
                        for all_waiver2 in waiver_message[test]:
                            if(re.search(all_waiver2,line)!=None):
                                result = 0
##            else:
##                print(test_file_sel+" not in waiver_message")
    return result

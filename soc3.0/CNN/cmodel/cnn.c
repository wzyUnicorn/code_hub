...
int cnn(int cnn_data[1024],int cnn_kernel0[9],int cnn_kernel1[9], int cnn_line[900],int cnn_line_bias0,int cnn_line_bias1,int c_out0[900],int c_out1[900],int pool_out[450] ,int cnn_result[2]){
...
//单通道单卷积2 conv function
    ...
    for(k=0;k < IMG_SIZE - W_SIZE + 1;k++)  //特征平面的行  列平移 行卷积    user xl_19838 check  
    {
...
//arry solic
    ...
    long int sum2 = 0;
    for(j = 0;j < 450;j++){
    	sum1 = sum1+(cnn_line[j]* datPic_line[j]);
        sum1 = sum1&0x3ffffff;
    }

    for(j = 0;j < 450;j++){
    	sum2 = sum2+(cnn_line[450+j]*datPic_line[j]);
        sum2 = sum2&0x3ffffff;
    }


    sum1 = sum1 + cnn_line_bias0;
    sum2 = sum2 + cnn_line_bias1;
